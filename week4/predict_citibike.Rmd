...
```
plor the number of trips with the min temperature on each day
```{r}
...
First, shuffle the data to 80% train and 20% validation split for the 90% of the sampling data. 
...
for(k in K){
  ...
  #evaluate on the training data(the 80% shuffled data)
  train_err[k] <- sqrt(mean((predict(model,trips_per_day_train)- trips_per_day_train$num_trips)^2))

  #evaluate on the validate data(20% of the shuffled data)
...
# notice that the line above means the model used for prediction. It can be or should be optimized
...
```
# k-fold cross-validation | RMSE validation data
```{r}
set.seed(42)
num_folds <- 5
num_days <- nrow(train_dataset)
...
trips_per_day <- train_dataset[ndx, ] %>%
...
##  Challenge- adding holiday effects. 
