...
year_proportion <- inner_join(year_counts, total_counts, by= c('year' = 'year')) %>% mutate('prop'= volume/total_volume,'term_factor' = factor(term) )
```
...
figure_3a %>% ggplot(aes(x= year, y= prop, color = term_factor)) + geom_line() + scale_y_continuous(labels = scales::percent)
...
Yes, the plot above shows the same thing on the paper. 
...
![](/Users\xiuwen\Pictures\Screenshots\Screenshot 2023-06-19 131523.png)
...
total_counts_over_time <- total_counts %>% filter(year>= 1850 & year <= 2012)

total_counts_over_time %>% ggplot(aes(x = year, y= total_volume))+ geom_line() + scale_y_continuous(labels = scales::comma)
...
The difference between b and d leads me to reevaluate the results because d is the count of the data, while b is the proportion. 
...
```{r compute-peaks}
# total_count_peak <- figure_3a %>% group_by(term) %>% arrange(term, desc(prop), year) %>%  top_n(1)
# 
# total_counts_peak

total_counts_peak <- figure_3a %>% group_by(term) %>% filter(year_proportion == max(year_proportion))
...
total_count_half_life <- figure_3a %>% 
  group_by(term) %>% 
  mutate(perk_proportion = max(year_proportion)) %>% 
  filter(year_proportion <= peak_proportion/2 & year> term) %>% 
  filter(year == min(year)) %>% 
  mutate(half_life = year -term)

total_count_half_life
```
...
half_life_all_years <- year_proportion %>% filter(year>= 1850& year<= 2012) %>% filter(term>=1875& term<=1975) %>% group_by(term) %>% mutate(peak_proportion = max(proportion_of_total)) %>% filter(year_proportion <= peak_proportion/2 & year> term) %>% 
  filter(year == min(year)) %>% mutate(half_life = year- term) %>% arrange(term)

half_life_all_years %>% ggplot(aes(x= term, y= half_life)) + geom_point() + geom_smooth()


total_count_half_life %>% arrange(half_life)

total_count_half_life %>% arrange(desc(half_life))
